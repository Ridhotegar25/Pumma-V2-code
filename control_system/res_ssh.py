...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe("Sebesi_ssh")  # Berlangganan ke topik Sebesi_ssh
    else:
...
def on_message(client, userdata, msg):
    try:
        ...
        if payload == "1":
            ...
            result = os.system("sudo systemctl restart ssh")
            if result == 0:
                print("SSH service restarted successfully.")
                client.publish("Sebesi_ssh_response", "success")  # Kirim pesan sukses ke broker
            else:
