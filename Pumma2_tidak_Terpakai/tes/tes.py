...
def mqtt_loop():
    """Loop MQTT untuk koneksi dan komunikasi."""
    global client
    client = mqtt.Client()
...
def save_log(data):
    """Simpan data ke file log dengan ts_human, maxbotic1, dan maxbotic2."""
    date_str = datetime.now().strftime("%d-%m-%Y")
    file_path = f"/home/pi/Data/Log_maxbo/Log_MB{date_str}.txt"
    try:
        ts_human = data.get("TS_Nano", "")
        maxbotic1 = data.get("maxbotic1", "")
        maxbotic2 = data.get("maxbotic2", "")
        
        # Format log: ts_human,maxbotic1,maxbotic2
        log_entry = f"{ts_human},{maxbotic1},{maxbotic2}\n"
